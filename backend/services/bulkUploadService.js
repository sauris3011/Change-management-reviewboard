...
function calculateSimpleRiskScore(changeData) {
  ...
  // Change type impact
  if (changeData.change_type === 'emergency') riskScore += 25;
  else if (changeData.change_type === 'normal') riskScore += 10;
  else riskScore += 5;

  // Category impact
  if (changeData.change_category === 'database') riskScore += 20;
  else if (changeData.change_category === 'infrastructure') riskScore += 15;
  else if (changeData.change_category === 'deployment') riskScore += 10;
  else riskScore += 5;

  // Complexity impact
  if (changeData.complexity === 'high') riskScore += 15;
  else if (changeData.complexity === 'medium') riskScore += 8;
  else riskScore += 3;
